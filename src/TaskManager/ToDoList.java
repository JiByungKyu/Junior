...
public class ToDoList extends JFrame implements ActionListener {
	...
	private static JTextField fieldDisplay;
	...
	public ToDoList()
	{
		...
		timeDisplay   = new JTextField();		timeDisplay.setEditable(false);			timeDisplay.setBackground(Color.white);
		priorityDisplay  = new JTextField();	priorityDisplay.setEditable(false);		priorityDisplay.setBackground(Color.white);
		fieldDisplay  = new JTextField();		fieldDisplay.setEditable(false);		fieldDisplay.setBackground(Color.white);
		...
		rightPanel.add(time);	rightPanel.add(timeDisplay);
		rightPanel.add(priority);	rightPanel.add(priorityDisplay);
		rightPanel.add(field);	rightPanel.add(fieldDisplay);
	...
	public static void changeOutputDetails(int index)
	{
		...
		startDateDisplay.setText(""+myTasks.get(index).getStartDate().toString());
		timeDisplay.setText(""+myTasks.get(index).getStartDate().getHour() + ":"+myTasks.get(index).getStartDate().getMin());
		priorityDisplay.setText(""+myTasks.get(index).getPriority());
		if(myTasks.get(index).getField())
			fieldDisplay.setText("Outdoor");
		else 
			fieldDisplay.setText("Inside");
		...
		try
		{
			...
			try {
				...
				while(textFile.hasNextLine())
				{
					...
					t.setName(str[0]);
					t.setStartDate(new myDate(Integer.parseInt(str[1]), Integer.parseInt(str[2]), Integer.parseInt(str[3]),	Integer.parseInt(str[4]), Integer.parseInt(str[5])));
					t.setPriority(str[6]);
